...
static const uint8_t u8aManagementFrame[] = {
	...
	0x32, 0x04, 0x30, 0x48, 0x60, 0x6c,//extended support rates
	...
	//HT Information
	0x3d, 0x16, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
...
int main(void) {
  ...
  sz = sizeof(u8aRadiotapHeader) + sizeof(struct ieee80211_hdr)+140;//management frame 140 bytes
   ...
   * the FC, hence 0x02.
   */
  fcchunk[0] = 0x80;//(( WLAN_FC_TYPE_CONTROL << 2) | (WLAN_FC_SUBTYPE_RTS << 4));
  fcchunk[1] = 0x00;
...
const uint8_t AP[6] = { 0x48, 0xee, 0x0c, 0xc3, 0x9e, 0x23 };
  ...
  memcpy(&hdr->addr1[0], BROADCAST, 6*sizeof(uint8_t));
