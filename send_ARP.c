...
static const uint8_t u8aRadiotapHeader[] = {
  ...
  0x15, 0x00, // <-- number of bytes in our header (count the number of "0x"s)
...
//  0x02, // <-- rate
  ...
  0x6c, 0x09, 0x80, 0x04, // <-- channel
  0xe5, 0x01,//ssi, antenna
...
const uint8_t arpllc[8] = { 0xaa, 0xaa, 0x03, 0x00, 0x00, 0x00, 0x08, 0x06 };

static const uint8_t u8aARPFrame[] = {
	//hardwire type
	0x00, 0x01, 
...
int main(void) {
  ...
  sz = sizeof(u8aRadiotapHeader) + sizeof(struct qos_hdr)+sizeof(arpllc)+28;//management frame 140 bytes
  buf = (uint8_t *) malloc(sz);

  /* Put our pointers in the right place */
  rt = (uint8_t *) buf;
  hdr = (struct qos_hdr *) (rt+sizeof(u8aRadiotapHeader));
  llc = (uint8_t *)(hdr+1);
  arp = (uint8_t *)(llc+sizeof(arpllc));
   ...
   * the FC, hence 0x02.
   */
  fcchunk[0] = 0x88;//(( WLAN_FC_TYPE_CONTROL << 2) | (WLAN_FC_SUBTYPE_RTS << 4));
  fcchunk[1] = 0x02;
...
/* The LLC+SNAP header has already been explained above */
  memcpy(llc, arpllc, 8*sizeof(uint8_t));

//management frame construction
  memcpy(arp, u8aARPFrame, sizeof(u8aARPFrame));
