...
int main(void) {
   ...
   * the FC, hence 0x02.
   */
  fcchunk[0] = 0xb4;//(( WLAN_FC_TYPE_CONTROL << 2) | (WLAN_FC_SUBTYPE_RTS << 4));
  fcchunk[1] = 0x00;
...
const uint8_t AP[6] = { 0x48, 0xee, 0x0c, 0xc3, 0x9e, 0x23 };
  ...
  hdr->duration_id = 0x022c;
  memcpy(&hdr->addr1[0], AP, 6*sizeof(uint8_t));
  memcpy(&hdr->addr2[0], STA, 6*sizeof(uint8_t));
